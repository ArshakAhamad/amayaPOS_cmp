...
router.delete('/productin/:id', async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query('DELETE FROM productin WHERE id = ?', [id]);
...
router.get('/product-returns', async (req, res) => {
  ...
  router.post('/product-returns', async (req, res) => {
    ...
    if (!returns || !Array.isArray(returns) || returns.length === 0) {
      return res.status(400).json({ success: false, message: 'No return data provided' });
    }
    ...
    try {
      ...
      for (const item of returns) {
        const { date, product_id, unitCost, quantity, totalCost, avgCost, stock } = item;
      ...
      res.status(201).json({ success: true, message: 'Product returns saved successfully' });
    } catch (err) {
      await pool.execute('ROLLBACK');
      console.error('Error saving product returns:', err);
      res.status(500).json({ success: false, message: 'Failed to save product returns' });
    }
  });
