...
router.post("/vouchers", async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      res
        .status(201)
        .json({ success: true, message: "Voucher created successfully." });
    } else {
      res
        .status(500)
        .json({ success: false, message: "Failed to create voucher." });
    }
  } catch (err) {
...
router.get("/vouchers", async (req, res) => {
  try {
    ...
    if (vouchers.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No vouchers found." });
    }
...
router.put("/vouchers/:id/cancel", async (req, res) => {
  try {
    ...
    if (voucher[0].status === "Cancelled") {
      return res
        .status(400)
        .json({ success: false, message: "Voucher is already cancelled." });
    }
    ...
    if (voucher[0].status === "Redeemed") {
      return res
        ...
        .json({
          success: false,
          message: "Redeemed vouchers cannot be cancelled.",
        });
    }
    ...
    if (result.affectedRows > 0) {
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to cancel voucher." });
    }
  } catch (err) {
