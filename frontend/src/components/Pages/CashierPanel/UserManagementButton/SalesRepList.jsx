...
const SalesRepList = () => {
  ...
  const totalPages = Math.ceil(filteredSalesReps.length / entriesPerPage);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setSalesReps((prevReps) =>
          prevReps.map((rep) =>
            rep.salesrep_id === currentRep.salesrep_id ? { ...rep, ...editFormData } : rep
          )
        );
  ...
  {isEditModalOpen && currentRep && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl">
            <div className="flex justify-between items-center border-b p-4">
              <h3 className="text-lg font-semibold">Edit Sales Representative</h3>
              <button 
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Sales Representatives</h2>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedSalesReps.length > 0 ? (
                  paginatedSalesReps.map((rep, index) => {
                    ...
                    return (
                      <tr key={rep.salesrep_id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <tr>
                    <td colSpan="8" className="px-6 py-4 text-center text-sm text-gray-500">
                      {salesReps.length === 0 ? 'No sales reps available' : 'No sales reps match your search'}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="text-sm text-gray-600">
            ...
            <span className="font-medium">{Math.min(currentPage * entriesPerPage, filteredSalesReps.length)}</span>{' '}
            of <span className="font-medium">{filteredSalesReps.length}</span> sales reps
          </div>
