  ...
  return (
    <div className="main-content p-6 flex flex-col items-center">
      ...
      <div className="sales-summary grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-6xl mt-6">
        ...
        <div className={`summary-card p-6 rounded-lg shadow-md flex flex-col items-center justify-center text-center border-2 hover:shadow-lg transition-shadow ${
          (data?.summary.profit || 0) >= 0 ? 'border-green-400' : 'border-red-400'
        }`}>
          <span className="icon text-3xl mb-2">⚡</span>
          <div>
            ...
            <h3 className={`text-lg font-semibold ${
              (data?.summary.profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
      ...
      <div className="sales-table bg-white p-6 rounded-lg shadow-md overflow-x-auto w-full max-w-6xl mt-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Receipt Wise Sales</h3>
          <span className="text-sm text-gray-500">
            {data?.receipts.length || 0} records found
          </span>
        </div>
          ...
          <table className="w-full table-auto border-collapse">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Sale</th>
              ...
              {data?.receipts && data.receipts.length > 0 ? (
                data.receipts.map((receipt, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-4 py-3 border-b text-sm text-gray-600">{index + 1}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600">{receipt.date}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600">{receipt.type}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600 font-medium">{receipt.reference}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600">LKR {formatCurrency(receipt.sale)}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600">LKR {formatCurrency(receipt.discount)}</td>
                    <td className="px-4 py-3 border-b text-sm text-gray-600">LKR {formatCurrency(receipt.cost)}</td>
                    <td className={`px-4 py-3 border-b text-sm font-medium ${
                      receipt.profit >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      LKR {formatCurrency(receipt.profit)}
                    </td>
                    <td className={`px-4 py-3 border-b text-sm ${
                      receipt.profit_percentage >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {receipt.profit_percentage.toFixed(2)}%
                    </td>
                  </tr>
                ))
              ) : (
