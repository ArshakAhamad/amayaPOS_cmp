      ...
      <div className="cashier-summary grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl">
        {cashierData.length > 0 ? (
          cashierData.map((cashier, index) => (
            <div key={index} className="cashier-card p-6 bg-white rounded-lg shadow-lg border border-gray-200 text-center flex flex-col justify-between">
              ...
              <div className="mt-5 bg-gray-100 p-4 rounded-md shadow-inner">
                ...
                <p className="text-red-600 font-semibold">Cash In Hand: LKR {cashier.cash.toFixed(2)}</p>
              </div>
            </div>
          ))
        ) : !loading && !error && (
